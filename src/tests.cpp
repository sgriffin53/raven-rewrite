...
int posMatch(Position *pos1, Position *pos2) {
	for (int i = PAWN;i < KING;i++) {
		if (pos1->pieces[i] != pos2->pieces[i]) {
			std::cout << "failed on piece\n";
			return 0;
		}
	}
	if (pos1->colours[WHITE] != pos2->colours[WHITE])  {
		std::cout << "failed on white\n";
		return 0;
	}
	if (pos1->colours[BLACK] != pos2->colours[BLACK]) {
		std::cout << "failed on black\n";
		return 0;
	}
...
void testMakeMove() {
	...
	std::cout << "White normal move test: ";
	makeMovestr("e2e4",&pos);
	Move testmove = {.from=E2, .to=E4, .prom=NONE, .cappiece=NONE, .type=DOUBLE };
	if ((getPiece(&pos,E4) == PAWN) && (getPiece(&pos,E2) == NONE)) {
		std::cout << "Make: Passed\n";
	}
	else std::cout << "Make: Failed\n";
	unmakeMove(&testmove, &pos);
	if (posMatch(&pos, &origpos)) {
		std::cout << "Unmake: Passed\n";
	}
	else std::cout << "Unmake: Failed\n";
	
	Move testmove1 = {.from=E2, .to=E4, .prom=NONE, .cappiece=NONE,.type=DOUBLE };
	Move testmove2 = {.from=E7, .to=E5, .prom=NONE, .cappiece=NONE, .type=DOUBLE };
	std::cout << "Black normal move test: ";
	makeMovestr("e2e4",&pos);
	...
	parsefen(&pos,"startpos");
	std::cout << "White en passant move test: ";
	...
	parsefen(&pos,"startpos");
	std::cout << "Black en passant move test: ";
	...
	std::cout << "White kingside castling test: ";
	...
	parsefen(&pos,"r3k2r/8/8/8/8/8/8/R3K2R w - -");
	std::cout << "White queenside castling test: ";
	...
	std::cout << "Black kingside castling test: ";
	...
	std::cout << "Black queenside castling test: ";
