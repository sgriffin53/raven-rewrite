...
U64 bishop_moves(const int sq, const U64 &occ) {
    ...
    return *(
        ...
        (((occ & bishop_masks[idx].mask_) * bishop_stuff[idx].first) >> 55));
}

U64 rook_moves(const int sq, const U64 &occ) {
    const int idx = sq;
    return *(magic_moves.data() + rook_stuff[idx].second +
             (((occ & rook_masks[idx].mask_) * rook_stuff[idx].first) >> 52));
}
